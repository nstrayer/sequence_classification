...
- The dialated convolution approach to CNNs is being used by google in their new [Wavenet](https://deepmind.com/blog/wavenet-generative-model-raw-audio/) architecture. 

- It seems to solve both the issues with CNNs (insensitivity to global order), while still being fast to train/run. 

![](http://sergeiturukin.com/assets/2017-02-23-155956_803x294_scrot.png)
...
- Building architectures that can handle heterogeneous input sources without any conversion to numerics.
    - Have a separate input layer on all data sources and allow the model to combine them.
...
- Train a model to explain. 
    - Similar to GANs: a discriminator and an explainer. 
