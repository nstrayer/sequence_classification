...
_Sometimes called sequence labeling._
---
class: middle
## Why not traditional methods?
...
While neural networks have been around for a long time, reasonable methods for training didn't really come around till 1986 with the introduction of the back propagation algorithm.
...
.center[
  <img height = 300 width = 475 src = figures/backprop.png />
]
...
.pull-left[
- Recurrent Nueral Networks (RNNs) deals with correlation in successive inputs by 'remembering' what it saw at the previous input. 
  
- Does this by feeding the values of its hidden layers to themselves at the next timestep. 
...
- This problem makes training RNNs to learn long-term time dependencies very hard. 
...
- LSTMs help control the exploading and vanishing gradient prblem by adding 'gates' that can decide to let through information/ control the flow of the gradient back in time. 
...
.pull-left.middeling[
- Slides feature detectors sequentially over data to detect features.

- Further layers aggregate those features and make decisions based on their occurance/position.
...
.small[What type of unsupervised model?]
  
Then we can train a supervised model on top of the unsupervised models layers.
]
